...
def compute_single_sp(G_gt_, G_prop_, kd_idx_dic_prop, kdtree_prop,
    ...
    [source_gt, target_gt] = np.random.choice(G_gt_.nodes(), size=2, replace=False)
    if verbose:
    ...
    if not nx.has_path(G_gt_, source_gt, target_gt):
        ...
        len_gt = nx.dijkstra_path_length(G_gt_, source_gt, target_gt, weight=weight)
        
    # get nodes in prop graph
    # see if source, target node exists in proposal
    source_p_l, _ = topo_metric.nodes_near_point(x_s_gt, y_s_gt, 
                                               kdtree_prop, kd_idx_dic_prop, 
                                               x_coord=x_coord, y_coord=y_coord, 
                                               radius_m=query_radius)
    target_p_l, _ = topo_metric.nodes_near_point(x_t_gt, y_t_gt, 
                                               kdtree_prop, kd_idx_dic_prop, 
                                               x_coord=x_coord, y_coord=y_coord, 
                                               radius_m=query_radius)
    ...
    if (len(source_p_l) == 0) or (len(target_p_l) == 0):
        ...
        if not nx.has_path(G_prop_, source_p, target_p):
            ...
            len_prop = nx.dijkstra_path_length(G_prop_, source_p, target_p, weight=weight)
    ...
    if make_plots:
        ...
        # plot initial graph
        if len_gt != -1:
            fig, ax = ox.plot_graph_route(G_gt_, nx.shortest_path(G_gt_, source=source_gt, target=target_gt, weight=weight))
        else:
            fig, ax = ox.plot_graph(G_gt_, axis_off=True)
        ...
        # plot proposal graph
        if len_prop != -1:
            fig, ax1 = ox.plot_graph_route(G_prop_, nx.shortest_path(G_prop_, source=source_p, target=target_p, weight=weight))
        else:
            fig, ax1 = ox.plot_graph(G_prop_, axis_off=True)            
...
def compute_sp(G_gt_, G_prop_, 
    ...
    kd_idx_dic_p, kdtree_p, pos_arr_p = topo_metric.G_to_kdtree(G_prop_)
...
if __name__ == "__main__":
    ...
    G_gt_init1 = ox.simplify_graph(G_gt_init.to_directed()).to_undirected()
