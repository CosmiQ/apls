...
###############################################################################
def get_node_positions(G_, x_coord='x', y_coord='y'):
...
def query_kd_ball_latlon(kdtree, kd_idx_dic, lat, lon, r_km):
    ...
    # check distances
    dists_km = np.asarray([distance_haversine(lat, lon, kdtree.data[i][1], 
...
###############################################################################
def nodes_near_point(x0, y0, kdtree, kd_idx_dic, 
                       x_coord='x', y_coord='y', radius_m=150, 
                       n_neighbors=-1,
                       verbose=False):
    ...
    if n_neighbors < 0:
        node_names, idxs_refine, dists_m_refine = query_kd_ball(kdtree, kd_idx_dic, 
                                                            point, radius_m)
    else:
        node_names, idxs_refine, dists_m_refine = query_kd_nearest(kdtree, 
                                                                   kd_idx_dic, 
                                                                   point, 
...
###############################################################################
def nodes_near_origin(G_, node, kdtree, kd_idx_dic, 
...
def ensure_radial_linestrings(G_sub_, origin_node, x_coord='x', y_coord='y',
    ...
    for i,(u, v, key, data) in enumerate(G_sub_.edges(keys=True, data=True)):   
        ...
        dist_to_start = scipy.spatial.distance.euclidean(origin_loc, geom_p_start)
    ...
    for u, v, data in G_.edges(data=True):
        ...
        if 'geometry' in data:
            ...
            for j,d in enumerate(interp_dists):
                ...
                # add to graph
                Gout, node_props, xn, yn = apls.insert_point(Gout, point, 
                                                        node_id=node_id,
...
def compute_single_topo(G_sub_gt_, G_sub_p_, 
    ...
    # get node positions
    pos_gt = get_node_positions(G_sub_gt_, x_coord=x_coord, y_coord=y_coord) 
    pos_p = get_node_positions(G_sub_p_, x_coord=x_coord, y_coord=y_coord) 
    
    # construct kdtree of ground truth
    kd_idx_dic0, kdtree0, pos_arr0 = G_to_kdtree(G_sub_gt_)
    ...
    for i,prop_point in enumerate(pos_p):
        ...
        # otherwise, check what's close
            ...
            dists_m = np.asarray([scipy.spatial.distance.euclidean(prop_point, 
                                    [kdtree0.data[itmp][0], kdtree0.data[itmp][1]])
                                        for itmp in idxs])
            # get min distance
    ...
    n_holes_filled = len(gt_tp)  # this should be the same as n_marbles_in_holes
...
def compute_topo(G_gt_, G_p_, subgraph_radius=150, interval=30, hole_size=5,
    ...
    kd_idx_dic, kdtree, pos_arr = G_to_kdtree(G_gt_)  
    # proposal graph kdtree
    kd_idx_dic_p, kdtree_p, pos_arr_p = G_to_kdtree(G_p_)
    ...
    for i,origin_node in enumerate(origin_nodes): 
        ...
        # get subgraph
        node_names, node_dists = nodes_near_origin(G_gt_, origin_node, 
                                                   kdtree, kd_idx_dic, 
        ...
        # insert points
        G_holes, xms, yms = insert_holes_or_marbles(G_sub, origin_node, 
        ...
        node_names_p, idxs_refine_p, dists_m_refine_p = query_kd_ball(kdtree_p, kd_idx_dic_p, 
                                                            origin_point, hole_size)
        if len(node_names_p) == 0:
            if verbose:
                print(("0oooops, no proposal node correspnding to", origin_node))
        ...
        # get subgraph
        node_names_p, node_dists_p = nodes_near_origin(G_p_, origin_node_p, 
        ...
        # make sure all nodes connect to origin
        node_names_conn_p = nx.node_connected_component(G_sub0_p, origin_node_p)
        ...
        # insert points
        G_holes_p, xms, yms = insert_holes_or_marbles(G_sub_p, origin_node_p, 
                                          interval=interval, n_id_add_val=1,
                                          verbose=False)
        ...
        true_pos_count, false_pos_count, false_neg_count, \
                precision, recall, f1 = compute_single_topo(G_holes, G_holes_p, 
                        x_coord=x_coord, y_coord=y_coord, 
        ...
        if i == 0 and make_plots:
            ...
            plt.close('all')
            # plot initial graph
            fig, ax = ox.plot_graph(G_gt_, axis_off=False)
            ...
            # plot subgraph
            fig2, ax2 = ox.plot_graph(G_sub, axis_off=False)
            ax2.set_title("Subgraph")
            plt.close('all')
        
            # plot G_holes
            fig, ax = ox.plot_graph(G_holes, axis_off=False)
            ...
            # plot initial graph
            fig, ax = ox.plot_graph(G_p_, axis_off=False)
            ...
            # plot subgraph
            fig2, ax2 = ox.plot_graph(G_sub_p, axis_off=False)
            ax2.set_title("Proposal Subgraph")
            plt.close('all')
        
            # plot G_holes
            fig, ax = ox.plot_graph(G_holes_p, axis_off=False, node_color='red')
            ...
            arr_marbles =  get_node_positions(G_holes_p, x_coord='x', y_coord='y')
            ax.scatter(arr_marbles[:,0], arr_marbles[:,1], c='red')
    
            # show gt node ids
            ax = apls_tools.plot_node_ids(G_holes, ax, fontsize=9)  # node ids
    
            # show prop node ids
            ax = apls_tools.plot_node_ids(G_holes_p, ax, fontsize=9)  # node ids
            plt.close('all')
...
if __name__ == "__main__":
    ...
    truth_dir = '/cosmiq/spacenet/data/spacenetv2/AOI_2_Vegas_Test/400m/gt_graph_pkls'
    prop_dir = '/tmps/graphs'
    ##########################
    ...
    #G_gt_init1 = ox.simplify_graph(G_gt_init0.to_directed()).to_undirected()
    #G_gt_init = ox.project_graph(G_gt_init1)
    ...
    G_p_init1 = ox.simplify_graph(G_p_init0.to_directed()).to_undirected()
    G_p_init = ox.project_graph(G_p_init1)
